...
export default function FormAddGoal({ submitAdd, clubId }) {
  ...
  const { data: playerData, refetch } = useQuery(
    ["player"],
    async () => await getPlayersByClubId(clubId),
  );
  ...
  useEffect(() => {
    if (watch("playerid"))
      setValue(
        "player",
        players.find((item) => item.id === watch("playerid")),
      );
    else setValue("player", players[0]);
    ...
    <>
      <form onSubmit={handleSubmit(submitAdd)} className="min-w-[300px]">
        <div className="form-group">
          ...
          <select {...register("playerid", {})} className="input-field">
        ...
        <div className="form-group !flex">
          ...
          <select
            ...
            className="ml-4 rounded border-2 border-green-300"
          >
            {Array.from(Array(rule?.goal.quantityType).keys()).map(
              (item, index) => (
        ...
        <div className="form-group !flex">
          ...
          <input className=" ml-4" type="checkbox" {...register("isOwnGoal")} />
        </div>
